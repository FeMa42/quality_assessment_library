...
from metrics.metrics_old import Metrics, GeometryMetrics, CarQualityMetrics
...
def process_metrics_by_viewpoint(ground_truth_folder, generated_folder, device="cuda"):
    ...
    global_metrics = Metrics(device=device, compute_distribution_metrics=True)

    # gather all image‐tensors
    all_gt = []
    all_gen = []
    for obj in obj_ids:
        for vp in sorted(viewpoint_set):
            gt_path  = os.path.join(ground_truth_folder, obj, vp)
            gen_path = os.path.join(generated_folder,    obj, vp)
            if not os.path.exists(gt_path) or not os.path.exists(gen_path):
                continue
            # load + preprocess with the same function your library uses
            img_gt  = preprocess_image(Image.open(gt_path).convert("RGB")).to(device)
            img_gen = preprocess_image(Image.open(gen_path).convert("RGB")).to(device)
            all_gt .append(img_gt)
            all_gen.append(img_gen)

    if all_gt:
        # stack into one big batch: shape [N, C, H, W]
        batch_gt  = torch.stack(all_gt,  dim=0)
        batch_gen = torch.stack(all_gen, dim=0)

        # update the distribution metrics
        _ = global_metrics.compute_image(batch_gen, batch_gt)

        # pull out the final FID/IS/KID
        dist = global_metrics.get_total_metrics()
        results["global_distribution_metrics"] = {
            "FID": convert_to_float(dist["FID"]),
            "IS" : convert_to_float(dist["IS"]),
            "KID": convert_to_float(dist["KID"]),
        }
    else:
        results["global_distribution_metrics"] = None
